...
SIHelper(@semilogx, idx, 'high', betas, T_critical, -0.4046, 0.1193)
figure(3); s3 = subplot(1, 3, 3); hold(s3, 'on');
SIHelper(@semilogx, idx, 'low', betas, T_critical, -0.4712, -0.2819)
...
SIHelper(@loglog, idx, 'high', betas, T_critical, 0.1108, 0.1447); ylim([0.75 1])
...
plot(1./betas(idx+1:end), (1-sinh(2*query.J*betas(idx+1:end)).^(-4)).^0.125, 's-.', 'DisplayName', 'Analytical Solution'); xlim([1 6.25])
