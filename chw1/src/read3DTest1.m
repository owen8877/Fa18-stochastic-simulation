...
for N = Ns
    ...
    figure(1)
    subplot(2, 3, 1); part1plotHelper(betas, means, query, true, 'Inner Energy', num2str(query.N))
    subplot(2, 3, 2); part1plotHelper(betas, betas.^2 .* vars, query, true, 'Specific Heat', num2str(query.N))
    subplot(2, 3, 3); part1plotHelper(betas, xiMeans, query, false, 'Characterisitic Length', num2str(query.N))
    subplot(2, 3, 4); part1plotHelper(betas, magMeans, query, true, 'Magneti...', num2str(query.N))
    subplot(2, 3, 5); part1plotHelper(betas, betas.^2 .* magVars, query, true, 'Magneti Kai...', num2str(query.N))
